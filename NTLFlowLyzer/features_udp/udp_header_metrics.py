...
class DeltaHeaderSizeMeanFeature(Feature):
    """Mean of header size differences between consecutive packets."""
    ...
    def extract(self, flow) -> float:
        packets = flow.get_packets()
        if len(packets) < self.min_delta_samples:
            return 0.0
        
        # Calculate actual header sizes for each packet
        header_sizes = []
        for pkt in packets:
            # Use actual packet length minus payload bytes to get real header size
            header_size = pkt.get_length() - pkt.get_payloadbytes()
            header_sizes.append(header_size)
        
        # Calculate absolute differences
        deltas = np.abs(np.diff(header_sizes))
        return float(np.mean(deltas)) if len(deltas) > 0 else 0.0

class DeltaHeaderSizeStdFeature(Feature):
    """Standard deviation of header size differences."""
...
class DeltaHeaderSizeMaxFeature(Feature):
    """Maximum header size difference between consecutive packets."""
    ...
    def extract(self, flow) -> float:
        ...
        # Calculate absolute differences
        deltas = np.abs(np.diff(header_sizes))
        return float(np.max(deltas)) if len(deltas) > 0 else 0.0 